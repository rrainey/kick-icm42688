...
typedef struct {
    ...
    uint8_t az_low;
    uint8_t temp;
} __attribute__((packed)) fifo_xfer_packet1;
...
typedef struct {
    ...
    uint8_t gz_low;
    uint8_t temp;
} __attribute__((packed)) fifo_xfer_packet2;
...
typedef struct  {
    ...
    uint8_t aygy_low;
    uint8_t azgz_low;
} __attribute__((packed)) fifo_xfer_packet4;
...
class ICM42688 {
   public:
    ...
    // returns 0x47 for ICM-42688-P, 0xDB for ICM-42688-V, something else for an etirely different IC,
    // or returns ICM42688_RETURN_ERR in the event of an I/O error
    uint8_t getChipID(void);

    void calibrateOffsetBias(float* dest1, float* dest2);
